...
export function isReactivePrimitive(
  ...
  return !!isReactiveValue(value) && Object.hasOwn(value, PRIMITIVE);
}
...
function createReactiveObject<T extends object>(value: T) {
  if (wrappedCache.has(value)) {
        ...
        if (Reflect.has(ref.current, prop)) {
          if (isPrimitive(ref.current[prop])) {
            if (!target[PRIMITIVE_CACHE].hasOwnProperty(prop)) {
              ...
              // ref?
              target[PRIMITIVE_CACHE][prop] = reactive(ref.current[prop]);
            }
            return target[PRIMITIVE_CACHE][prop];
          } else {
